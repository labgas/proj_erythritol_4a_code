%% ery_4a_firstlevel_m6r_s2_fit_model.m
%
...
% check whether ery_4a_firstlevel_m6r_s1_options_dsgn_struct has been run
...
for sub=1:size(derivsubjs,1)
    ...
    for run=1:size(fmriprep_noisefiles,1)
        ...
        idx_nodir = ~[rundirlist(:).isdir]';
        rundirlist = [rundirlist(idx_nodir).name]; % tidied this up
            ...
            elseif ~contains(rundirlist,derivimgs{run}(1,1:end-3)) % .unzipped version
                copyfile(fullfile(subjderivdir,derivimgs{run}),fullfile(rundir,derivimgs{run}));
                gunzip(fullfile(rundir,derivimgs{run}));
                delete(fullfile(rundir,derivimgs{run}));
            end
        ...
        % omit trials that coincide with spikes if that option is chosen
            ...
            elseif strcmpi(LaBGAS_options.mandatory.omit_spike_trials,'no')==1
                ...
                error('\ninvalid LaBGAS_options.mandatory.omit_spike_trials option %s set in ery_4a_firstlevel_m6r_s1_options_dsgn_struct.m',LaBGAS_options.mandatory.omit_spike_trials)
            end
        ...
        % add pmods to structures for conditions of interest
            ...
            pmods_demean_cond = cell(1,size(DSGN.pmods{run},2));
                ...
                switch LaBGAS_options.pmods.pmod_type
                    ...
                    otherwise
                        error('\nInvalid LaBGAS_options.pmods.pmod_type option %s specified in ery_4a_firstlevel_m6r_s1_options_dsgn_struct, please check before proceeding',LaBGAS_options.pmods.pmod_type)
                end   
