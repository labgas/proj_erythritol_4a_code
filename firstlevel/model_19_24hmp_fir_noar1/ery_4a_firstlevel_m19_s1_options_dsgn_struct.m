%%% ery_4a_firstlevel_m19_s1_options_dsgn_struct.m
%
...
LaBGAS_options.display.plotdesign = false;
...
% INPUT
    ...
    % REQUIRED FIELDS
    DSGN.metadata = "proj-erythritol_4a first level analysis model 19, i.e. modeling 4 conditions for sucrose, erythritol, sucralose, with 24 hmp, finite impulse response, without ar(1)"; % field for annotation with study info, or whatever you like
    DSGN.modeldir = '/data/proj_erythritol/proj_erythritol_4a/firstlevel/model_19_24hmp_fir_noar1'; % directory where you want to write first level results for this model
...
%     c=c+1;DSGN.pmods{c}={{}};
    ...
    DSGN.convolution.type = 'fir'; % default hrf, which means canonical hrf - other options: fir, spline (the latter is not yet implemented @LaBGAS, help needed from Tor/Martin/Bogdan)
    DSGN.convolution.windowlength = 10; % in seconds
    DSGN.convolution.order = 5; % nr of timebins for fir model
    % NOTE: seems like canlab_glm_subject_level_run1subjects zeros out
    % duration automatically when type = fir
...
%     DSGN.singletrialsall = false; % default: false; if true, set DSGN.singletrials to true for all conditions
    DSGN.modelingfilesdir = 'model_19_24hmp_fir_noar1'; % name of subfolder which will be created within directory containing functional files where .mat files containing fields of DSGN structure will be saved; typically same as the last part of the path of DSGN.modeldir
...
% CONTRASTS
    ...
    DSGN.contrastweights{c} = eye(DSGN.convolution.order);
    c=c+1;
