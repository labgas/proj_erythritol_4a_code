% ery_4a_firstlevel_m6st_s1_options_dsgn_struct.m
%
...
% INPUT
    ...
    % REQUIRED FIELDS
    DSGN.metadata = "proj-erythritol_4a first level analysis single trial model 6st, i.e. modeling 4 conditions for sucrose, erythritol, sucralose, and water as long events (= duration of solution in mouth), with 12 hmp, canonical hrf, with ar(1)"; % field for annotation with study info, or whatever you like
    DSGN.modeldir = '/data/proj_erythritol/proj_erythritol_4a/firstlevel/model_6st_long_12hmp_can_ar1'; % directory where you want to write first level results for this model
...
%     DSGN.convolution.dispersion: default 0, which means no dispersion derivative
    ...
    DSGN.singletrials = {{1 1 1 1 0 0}}; % a cell array (1 cell per session) of cell arrays (1 cell per condition) of (corresponding to DSGN.conditions) of true/false values indicating whether to convert specified condition to set of single trial conditions
%     DSGN.singletrialsall = false; % default: false; if true, set DSGN.singletrials to true for all conditions
    DSGN.modelingfilesdir = 'model_6st_long_12hmp_can_ar1'; % name of subfolder which will be created within directory containing functional files where .mat files containing fields of DSGN structure will be saved; typically same as the last part of the path of DSGN.modeldir
